...
//入口
- (void)encodeFrame:(CGImageRef)imageRef andTime:(CMTime)prestime
{
    @synchronized(self)
    {
        if (_needParams)
        {
            ...
            _needParams = NO;
            if ([_headerWriter encodeFrame:imageRef andTime:prestime])
            {
    ...
    @synchronized(self)
    {
        ...
        [_writer encodeFrame:imageRef andTime:prestime];
    }
}
